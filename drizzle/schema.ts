...
// Advanced Reading System Tables (Admin-Only, Parallel Implementation)
// These tables are completely separate from standard readings tables
export const advancedReadings = mysqlTable("advancedReadings", {
  ...
  // Standard analysis (same as regular readings)
  executiveSummary: text("executiveSummary"),
  detailedAnalysis: text("detailedAnalysis"),
  stunningInsights: text("stunningInsights"),
  ...
  decadeTimeline: text("decadeTimeline"), // Decade-by-decade life timeline
  
  pdfPath: varchar("pdfPath", { length: 512 }),
  accuracyRating: int("accuracyRating"),
  errorMessage: text("errorMessage"),
  processingTimeSeconds: int("processingTimeSeconds"), // Track processing time
  tokensUsed: int("tokensUsed"), // Track Claude API token usage
});
...
export const advancedImages = mysqlTable("advancedImages", {
  id: varchar("id", { length: 64 }).primaryKey(),
  advancedReadingId: varchar("advancedReadingId", { length: 64 }).notNull(),
  imageType: mysqlEnum("imageType", [
    "frontal_neutral",
    "frontal_smile",
    "left_profile",
